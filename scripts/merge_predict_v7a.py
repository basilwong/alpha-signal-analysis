"""
V7a Merge + Predict (standalone).

Usage:
    modal run scripts/merge_predict_v7a.py
"""
...
app = modal.App("quantum-alpha-v7a-predict")
...
ADAPTER_PATH = "/outputs/quantum-alpha-openreasoning-7b-v7a/checkpoint-100"
MERGED_OUTPUT = "/outputs/openreasoning-7b-v7a-merged"
EVAL_FILE = "/outputs/articles_eval.jsonl"
PREDICTIONS_FILE = "/outputs/predictions_openreasoning7b_v7a.jsonl"
...
def merge_and_predict():
    ...
    test_outputs = llm.generate(test_prompts, SamplingParams(temperature=0.3, top_p=0.9, max_tokens=3000))
    sanity_pass = 0
    for i, output in enumerate(test_outputs):
        ...
        if "{" in parse_text and "}" in parse_text:
            try:
                json.loads(re.sub(r',\s*([}\]])', r'\1', parse_text[parse_text.find("{"):parse_text.rfind("}")+1]))
                sanity_pass += 1
                print(f"  Sanity {i}: OK")
            except:
                print(f"  Sanity {i}: JSON FAIL")
        else:
            print(f"  Sanity {i}: NO JSON")
