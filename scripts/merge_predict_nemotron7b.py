"""
Merge OpenReasoning-Nemotron-7B adapter and run vLLM predictions.

Usage:
    modal run scripts/merge_predict_nemotron7b.py
"""
...
app = modal.App("quantum-alpha-nemotron7b-predict")
...
ADAPTER_PATH = "/outputs/quantum-alpha-openreasoning-7b/checkpoint-100"
MERGED_OUTPUT = "/outputs/openreasoning-7b-merged"
EVAL_FILE = "/outputs/articles_eval.jsonl"
OUTPUT_FILE = "/outputs/predictions_openreasoning7b_v4.jsonl"
...
def merge_and_predict():
    """Merge adapter, then run vLLM predictions."""
    import torch
    ...
    if not os.path.exists(MERGED_OUTPUT + "/config.json"):
        ...
        print("Step 1: Merged model already exists, skipping.")

    # Step 2: Run vLLM predictions
    print("\nStep 2: Loading vLLM...")
    ...
    print(f"Time: {total_time/60:.1f} min, avg {total_time/len(prompts):.1f}s/article")
